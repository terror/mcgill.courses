...
OUTPATH = os.path.join("..", "..", "client", "src", "assets")
    ...
    try:
        ...
        for row in values:
            ...
            result[course_id].append(
                {"term": expand_term(course_term), "average": course_avg}
            )

        with open(os.path.join(OUTPATH, "courseAveragesData.json"), "w") as json_file:
