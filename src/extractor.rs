...
impl Extractor {
  ...
  pub(crate) fn run(&self, source: PathBuf) -> Result {
    ...
    while let Some(listings) =
      self.parse_course_listing_pages(self.aggregate_urls(page, page + self.batch_size))?
    {
      courses.extend(
        listings
          .par_iter()
  ...
  fn parse_course_listing_pages(&self, pages: Vec<Page>) -> Result<Option<Vec<CourseListing>>> {
    Ok(
      pages
        .par_iter()
        .flat_map(|page| {
  ...
  fn parse_course_listing_page(&self, page: &Page) -> Result<Option<Vec<CourseListing>>> {
    log::info!("Parsing html on page: {}...", page.number);

    let listings = parser::parse_course_listing_page(&Html::parse_fragment(&page.content()?))?;
      ...
      return Ok(Some(
        listings
          ...
          .map(|listing| CourseListing {
            url: format!("{}{}", Extractor::BASE_URL, listing.url),
            ..listing.clone()
          })
          .collect(),
      ));
  ...
  fn parse_course(&self, listing: CourseListing) -> Result<Course> {
    ...
    let course_page = parser::parse_course_page(Html::parse_fragment(&listing.content()?))?;
    ...
    let course = Course {
      ...
      faculty_url: format!("{}{}", Extractor::BASE_URL, course_page.faculty_url),
